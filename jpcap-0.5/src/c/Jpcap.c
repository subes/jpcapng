  ...
  (JNIEnv *env, jclass cl)
{
    ...
    for(i=0,d=alldevs;d;d=d->next)
    {
		...
		//set mac
#ifdef WIN32
		(*env)->SetByteArrayRegion(env,mac,0,6,IfTable->table[i].bPhysAddr);
#else
    /* make socket */
...
#ifdef WIN32
	  sprintf_s(pcap_errbuf[id],PCAP_ERRBUF_SIZE,"Unable to obtain  the net number or the netmask of device %s",dev);
#else
	  sprintf(pcap_errbuf[id],"Unable to obtain  the net number or the netmask of device %s",dev);
#endif
	pcds[id]=NULL;
  }
...
void get_packet(struct pcap_pkthdr header,u_char *data,jobject *packet,int id){
  ...
  if(clen>header.caplen-datalink_hlen(id)) clen=header.caplen-datalink_hlen(id);
  data+=hlen;
  clen-=hlen;
