...
jmethodID deviceConstMID,addressConstMID,handleMID,setPacketValueMID,setDatalinkPacketMID,
  ...
  // !!!!!!!! updated by Manu Bachimon 1st December 2006 : able to specify TCP and IP option and checksumfield
  getTcpOptionsMID, getIPv4OptionsMID, getTcpChecksumMID, getIPv4ChecksumMID, getUdpChecksumMID;
			   ...
			   jboolean opt)
{
  ...
  if (err != NULL) {
    char buf[2048];
#ifdef WIN32
// Manu modification strcpy_s(buf, 2048,"Error occurred while compiling or setting filter: ");
// Manu modification strncat_s(buf, 2048, err, _TRUNCATE);
#else
...
void set_Java_env(JNIEnv *env){
  ...
  setIPValueMID=(*env)->GetMethodID(env,IPPacket,"setIPv4Value",
		 "(BBBZZZBZZZSSSSS[B[B)V");
  setIPv4OptionMID=(*env)->GetMethodID(env,IPPacket,"setIPv4Options","([B)V");
  // *******************************
  ...
  setTCPValueMID=(*env)->GetMethodID(env,TCPPacket,"setValue","(IIJJZZZZZZBZZISS)V");
  // !!!!!!!! end
  // *******************************
  setTCPOptionMID=(*env)->GetMethodID(env,TCPPacket,"setTcpOptions","([B)V");
  ...
  // !!!!!!!! updated by Manu Bachimon 1st December 2006 : able to specify IP and TCP option fields
  getTcpOptionsMID=(*env)->GetMethodID(env,TCPPacket,
					       "getTcpOptions",
					       "()[B");
  getIPv4OptionsMID=(*env)->GetMethodID(env,IPPacket,
					       "getIPv4Options",
					       "()[B");
  getTcpChecksumMID=(*env)->GetMethodID(env,TCPPacket,
					       "getTcpChecksum",
					       "()[S");
  getIPv4ChecksumMID=(*env)->GetMethodID(env,IPPacket,
					       "getIPv4Checksum",
					       "()[S");
  getUdpChecksumMID=(*env)->GetMethodID(env,UDPPacket,
					       "getUdpChecksum",
					       "()[S");
