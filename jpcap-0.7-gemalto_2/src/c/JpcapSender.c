...
int set_packet(JNIEnv *env, jobject packet,char *pointer,int include_datalink)
{
  int length=0,dthlen=0, optlen=0, ipv4OptionLength=0;
  jbyteArray data, ipv4Options, tcpOptions;
  jshortArray ipv4Checksum;
  ...
  if(IsInstanceOf(packet,IPPacket))
  {
    ...
    if(ver==4)
    {
      ipv4Options=GetObjectField(IPPacket,packet,"[B","ipv4Options");
      if(ipv4Options == NULL)
      {
        optlen = 0;
        ipv4OptionLength = 0;
      ...
      {
        optlen=(*env)->GetArrayLength(env,ipv4Options);
        ipv4OptionLength = optlen;
      }
      length+=optlen;
      // !!!!!!!! End of modifications done by Manu Bachimon
		  ...
		  length+=IPv4HDRLEN;
      // !!!!!!!! updated by Manu Bachimon
      pointer+=(IPv4HDRLEN+ipv4OptionLength);
      // !!!!!!!! End of modifications done by Manu Bachimon
    }
    else
    {
    ...
    if(IsInstanceOf(packet,TCPPacket))
    {
      ...
      if(ver==4)
      {
        ip->ip_p=IPPROTO_TCP;
        ip->ip_len=length;
        set_tcp(env,packet,pointer,data,(void *)ip);
#ifdef INET6
      }
      else
      {
        ipv6->ip6_nxt=IPPROTO_TCP;
        ipv6->ip6_plen=length-IPv6HDRLEN;
        set_tcp(env,packet,pointer,data,(void *)ipv6);
#endif
      }
    }

    // UDP Packet in IP Packet
    ...
    else
    {
      if(ver==4)
      {
        ...
        (*env)->GetByteArrayRegion(env,data,0,length-IPv4HDRLEN,pointer);
#ifdef INET6
      }
      else
      {
        ipv6->ip6_plen=length-IPv6HDRLEN;
        (*env)->GetByteArrayRegion(env,data,0,length-IPv6HDRLEN,pointer);
#endif
      }
    }
...
#endif
      ip->ip_sum=0;
      // !!!!!!!! updated by Manu Bachimon 1st December 2006 : able to specify IP option and compute checksum according
      ipv4Checksum=(*env)->CallObjectMethod(env,packet,getIPv4ChecksumMID);
      if(ipv4Checksum == NULL)
      {
        ip->ip_sum=in_cksum((u_short *)ip,(20+ipv4OptionLength));
      }
      else
      {
        (*env)->GetShortArrayRegion(env,ipv4Checksum,0,1,&ip->ip_sum);
  ...
  {
    (*env)->GetByteArrayRegion(env,data,0,length,pointer);
  }
...
/**
Checksum for TCP or UDP packet on IPV4 or IPV6 (by G. Stornello)
parameters:
 - ip: pointer on structure IPV4 or IPV6 including the TCP or UDP packet
 - data: pointer on payload of IP packet (ie TCP or UDP packet)
 - size: size of data
**/
unsigned short in_cksum2(void *ip,unsigned short *data,int size)
{
  unsigned long sum = 0;
  u_short *p;
  int i;
  struct ip *ipv4 = (struct ip *) ip;
  struct ip6_hdr *ipv6 =  (struct ip6_hdr *) ip;

  // Pseudo header (specific to the IP version)

  if(ipv4->ip_v == 4)
  {
    // IPV4 case

    //  - Source and destination addresses (2*4bytes)
    p = (u_short *) &ipv4->ip_src;
    for(i=0; i<4; i++)
    {
      sum+=*(p+i);
    }
    ...
    //  - PTCL
    sum += htons((u_short)(ipv4->ip_p));

    //  - TCP Length (16 bits)
    sum += htons((u_short)size);
  }
  ...
  else
  {
    ...
    //  - Source and destination addresses (2*16bytes)
    p = (u_short *) &ipv6->ip6_src;
    for(i=0; i<16; i++)
    {
      sum+=*(p+i);
    }
    ...
    sum += htons(ipv6->ip6_plen);

    //  - Next Header (6=TCP)
    sum +=  htons((u_short)(ipv6->ip6_nxt));
  }

  // TCP header and paylod
  while(size > 1)
  {
    sum += *(data++);
    size -= 2;
  ...
  // Return the complement of checksum
  return (u_short)(~sum);
}
