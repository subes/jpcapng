...
public class IPPacket extends Packet {
  ...
  /** The version that should appear in the version field of the
   * IP packet.
   * 
   * By default, this value is set to -1, and the actual packet version
   * appears in the field (this.version).
   * 
   * Normally, there is no need to set this attribute.
   * 
   * Unlike this.version, this attribute does not determine the format
   * of the generated packet.
   */
  public short advertisedVersion;
  ...
  /** Protocol number for IPv4 in IP */
  public static final short IPPROTO_IPv4 = 4;
  ...
  /** Protocol number for IPv6 in IP */
  public static final short IPPROTO_IPv6 = 6;
  ...
  /** 
   * IPv4 checksum, if null checksum will computed automatically, 
   * otherwise the specified value at index 0 will be included into IP packet
   */
  public short[] ipv4Checksum;
  ...
  void setIPv4Value(byte ver, byte ihl, byte pri, boolean d, boolean t,
      boolean r, byte rsv_tos, boolean rf, boolean df, boolean mf,
      short offset, short len, short ident, short ttl, short proto, byte[] src,
      byte[] dst) {
  ...
  public void setIPv4Options(byte[] ipv4Options) {
    byte nbOfPaddingByte = 0;

    if (ipv4Options == null) {
      System.out.println("ERROR : ipv4Options[] is NULL");
      return;
    }
    // adding padding bytes if necessary
    nbOfPaddingByte = (byte)(ipv4Options.length % 4);
    this.ipv4Options = new byte[(ipv4Options.length + nbOfPaddingByte)];
    for (byte i = 0; i < ipv4Options.length; i++)
      this.ipv4Options[i] = ipv4Options[i];
  }
  ...
  void setIPv6Value(byte ver, byte v6class, int flow, short payload, byte nxt,
      short hlim, byte[] src, byte[] dst) {
    this.version = ver;
  ...
  public String toString() {
    if (version == 4) {
      return super.toString() + " " + src_ip + "->" + dst_ip + " protocol("
          + protocol + ") priority(" + priority + ") " + (d_flag ? "D" : "")
          + (t_flag ? "T" : "") + (r_flag ? "R" : "") + " hop(" + hop_limit
          + ") " + (rsv_frag ? "RF/" : "") + (dont_frag ? "DF/" : "")
          + (more_frag ? "MF" : "") + " offset(" + offset + ") ident(" + ident
          + ")";
    } else {
      return super.toString() + " " + src_ip + "->" + dst_ip + " protocol("
          + protocol + ") priority(" + priority + ") flowlabel(" + flow_label
          + ") hop(" + hop_limit + ")";
    }
  }

  public short[] getIPv4Checksum() {
    return this.ipv4Checksum;
  }
}
