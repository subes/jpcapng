...
void set_tcp(JNIEnv *env,jobject packet,char *pointer,jbyteArray data,struct ip *ip)
{
  ...
  if(tcpChecksum == NULL) {
    tcp->th_sum=in_cksum2(ip,htons((u_short)length+TCPHDRLEN+optlen),(u_short *)tcp,length+TCPHDRLEN+optlen);
  }
  else {
      (*env)->GetShortArrayRegion(env,tcpChecksum,0,1,(u_short *)tmpChksum);
      #if BYTE_ORDER == BIG_ENDIAN
        tcp->th_sum = tmpChksum[0];
      #endif
      #if BYTE_ORDER == LITTLE_ENDIAN 
        // swap nibbles
        tcp->th_sum = (((tmpChksum[0] & 0xff00) >> 8 ) | ((tmpChksum[0] & 0x00ff) << 8 ));
      #endif
