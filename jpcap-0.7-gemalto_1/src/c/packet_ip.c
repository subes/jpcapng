...
void set_ip(JNIEnv *env,jobject packet,char *pointer){
  ...
  // !!!!!!!! updated by Manu Bachimon
  {
	  ...
	  advertisedVersion = GetShortField(IPPacket,packet,"advertisedVersion");
	  if (advertisedVersion == -1) 
	  {
        ip->ip_v=GetByteField(IPPacket,packet,"version");
	  }
	  else
	  {
	    ip->ip_v=(unsigned char)advertisedVersion;
      }
  }
  ...
  ip->ip_off=((GetBooleanField(IPPacket,packet,"rsv_frag")?IP_RF:0)+
    ...
    GetShortField(IPPacket,packet,"offset")); // Fixed by patrice.lacouture@gemalto.com: unnecessary htons()
  ...
  // !!!!!!!! updated by Manu Bachimon 1st December 2006 : able to specify IP option field

  // Get IP option bytes
  ipv4Options=(*env)->CallObjectMethod(env,packet,getIPv4OptionsMID);
  if(ipv4Options != NULL) {
      optlen=(*env)->GetArrayLength(env,ipv4Options);
      (*env)->GetByteArrayRegion(env,ipv4Options,0,optlen,(char *)&ip->ip_options);

      // updating Internet Header Length (IHL) field according to options length
      ip->ip_hl+=(optlen/4);  
  }
  // !!!!!!!! End of modifications done by Manu Bachimon
