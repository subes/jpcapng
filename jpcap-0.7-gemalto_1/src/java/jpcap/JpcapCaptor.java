    ...
    static {
        try {
            System.load(System.getProperty("user.dir")
                    + "./lib/jpcap/lib/jpcap.dll");
        } catch (UnsatisfiedLinkError linkErr1) {
            try {
                System.loadLibrary("jpcap");
            } catch (UnsatisfiedLinkError linkErr2) {
                System.out
                        .println("unable to load jpcap.dll, make sure of the presence of this file");
            } catch (SecurityException secuEx1) {
                System.out
                        .println("security exception during the loading of jpcap.dll");
            }
        } catch (SecurityException secuEx2) {
            System.out
                    .println("security exception during the loading of jpcap.dll");
        }
    }
