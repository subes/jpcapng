...
void set_tcp(JNIEnv *env,jobject packet,char *pointer,jbyteArray option,jbyteArray data,struct ip *ip)
{
  ...
  tcp->th_off = 5 + (option_length + 4 - 1) / 4;
  ...
  if(data_length+IPv4HDRLEN+TCPHDRLEN>MAX_PACKET_SIZE)
    data_length=MAX_PACKET_SIZE-IPv4HDRLEN-TCPHDRLEN;
  
  //Option fix by Michal Spe 9.5.2008
  (*env)->GetByteArrayRegion(env,option,0,
			     option_length,
			     (u_char *)(pointer+TCPHDRLEN));
  (*env)->GetByteArrayRegion(env,data,0,
			     data_length,
			     (u_char *)(pointer+TCPHDRLEN+option_length));

  tcp->th_sum=0;
  tcp->th_sum=in_cksum2(ip,
			htons((u_short)data_length+TCPHDRLEN+option_length),
			(u_short *)tcp,
			data_length+TCPHDRLEN+option_length);
