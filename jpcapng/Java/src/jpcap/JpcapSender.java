...
public class JpcapSender extends JpcapInstance {
    ...
    private native void nativeSendPacket(Packet packet) throws IOException;

    private native void nativeCloseDevice();

    private JpcapSender() throws IOException {
        if (reserveID() < 0)
            throw new IOException("Unable to open a device: " + MAX_NUMBER_OF_INSTANCES
                    + " devices are already opened.");
    }
